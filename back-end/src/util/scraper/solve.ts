...
async function solve(page: puppeteer.Page): Promise<string|null> {
    try {
        await page.waitForFunction(() => {
            const iframe = document.querySelector("iframe[src*=\"api2/anchor\"]") as HTMLIFrameElement;
        ...
        await page.waitForFunction(() => {
            const iframe = document.querySelector("iframe[src*=\"api2/bframe\"]") as HTMLIFrameElement;
            if (!iframe) return false;

            const img = iframe.contentWindow.document.querySelector(".rc-image-tile-wrapper img") as HTMLImageElement;
        ...
        while (true) {
            ...
            const audioLink = await page.evaluate(() => {
                const iframe = document.querySelector("iframe[src*=\"api2/bframe\"]") as HTMLIFrameElement;
                return (iframe.contentWindow.document.querySelector("#audio-source") as HTMLAudioElement).src;
            });
            ...
            try {
                ...
                return page.evaluate(() => (document.getElementById("g-recaptcha-response") as HTMLTextAreaElement).value);
            } catch (e) {
                console.error(e);
            }
        }
